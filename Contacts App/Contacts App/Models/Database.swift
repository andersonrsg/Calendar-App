...
final class Database {
    ...
    func addContact(contact: RContact, success: () -> Void, failure: (String) -> Void) {
    ...
    func fetchContacts(success: ([RContact]) -> Void, failure: (String) -> Void) {
        
        let contacts = db?.objects(RContact.self).sorted(byKeyPath: "firstName", ascending: true)
        
        success(contacts?.toArray(type: RContact.self) ?? [])
        
    }
    
    func removeContact(contact: RContact?, success: () -> Void, failure: (String) -> Void) {
    ...
    func updateContact(contact: RContact,
                       email: REmail? = nil,
                       address: RAddress? = nil,
                       phone: RPhone? = nil,
                       index: Int,
                       success: (() -> Void)? = nil,
            ...
            try db?.write {
                ...
                if let email = email {
                    ...
                    if contact.addresses.count == index {
                        contact.addresses.append(RAddress())
                    }
